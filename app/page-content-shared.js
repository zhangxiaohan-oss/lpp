...
export function getDefaultPageContent() {
  return {
    ...
    categories: categories.map((item) => ({
      ...item,
      productSlugs: Array.isArray(item.productSlugs) ? item.productSlugs : []
    })),
...
export function mergeList(savedList, defaultList) {
...
export function mergePageContent(saved = {}) {
...
export function contentTimestamp(value) {
  return Number(asObject(value)._updatedAt || 0);
}

export function shouldUseRemoteContent(localRaw, remoteRaw) {
